...
$driverName = "HP PageWide MFP P57750 PCL 6" # Must be exact! Install driver on your computer and copy its name and paste here!
...
#FD
# Variables
$driverDownloadPath = '\\SYSADMIN01\Tools\Printers\Clubs\Front Desk\HP LaserJet M402n & M403n\Driver'
# Get the first non-loopback IPv4 address
$localIP = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object {
    $_.InterfaceAlias -notlike "*Loopback*" -and $_.PrefixOrigin -ne "WellKnown"
}).IPAddress
...
$driverName = "HP LaserJet Pro M402-M403 n-dne PCL-6" # Must be exact! Install driver on your computer and copy its name and paste here!
# Do not modify these variables
$portExists = Get-Printerport -Name $portname -ErrorAction SilentlyContinue
$printerExists = Get-Printer -Name $printerName -ErrorAction SilentlyContinue
#####################
